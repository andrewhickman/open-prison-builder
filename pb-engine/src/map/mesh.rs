...
use polyanya::{
    ...
    geo::{Area, BooleanOps, Closest, ClosestPoint, MultiPolygon, Point, Polygon, unary_union},
};
...
pub fn update_mesh(
    ...
    for map in &mut map_q {
        ...
        let mut polygons = exterior.difference(&interior);

        // remove degenerate polygons
        polygons
            .0
            .retain(|polygon| polygon.unsigned_area() > Pawn::AREA);

        let mut mesh = Mesh {
            layers: polygons
                .iter()
                .map(|polygon| Triangulation::from_geo_polygon(polygon.clone()).as_layer())
                .collect(),
            search_delta: RADIUS / 4.,
            search_steps: 2,
        };
        ...
        commands.entity(map.id()).insert(MapMesh { mesh, polygons });
    }
...
impl MapMesh {
    ...
    fn closest_point(&self, point: Vec2) -> Option<Coords> {
        ...
        match self.polygons.closest_point(&Point::new(point.x, point.y)) {
